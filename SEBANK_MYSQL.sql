...
BEGIN
   ...
   SELECT LAISUAT INTO v_LAISUAT FROM LOAI_SOTK WHERE KYHAN = p_KYHAN AND TRANGTHAI = 1;
    ...
    INSERT INTO SOTIETKIEM (MASO, MAKH, SODU, LOAI, NGAYMOSO, TRANGTHAI, LAISUAT) 
    VALUES (p_MASO, v_MAKH, p_SOTIEN, v_LOAITK, p_NGAYMOSO, 1, v_LAISUAT);
...
BEGIN
    ...
    SELECT NGAYMOSO, SODU, LOAI INTO v_NGAYMOSO, v_SODU, v_LOAITK FROM SOTIETKIEM WHERE MASO = p_MASO;
    ...
    -- Tính số tiền đã gửi trong hơn 15 ngày
    SELECT COALESCE(SUM(TienGui), 0) INTO v_SOTIENGUIHOPLE
    FROM PHIEUGUITIEN
    WHERE MASOTK = p_MASO
      AND DATEDIFF(p_NGAYRUT, NgayGui) >= 15;

    -- Tính tổng số tiền hợp lệ để rút
    SET v_SOTIENHOPLE = v_SODU + v_SOTIENGUIHOPLE;
   	IF p_SOTIEN > v_SOTIENHOPLE THEN
   		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Số tiền cần rút vượt quá số tiền được cho phép';
    END IF;
