...
BEGIN
    ...
    IF p_NGAYGUI > (SELECT NGAYGUIGANNHAT FROM SOTIETKIEM WHERE MASO = p_MASO) THEN
        UPDATE SOTIETKIEM
...
BEGIN
    ...
    LEFT JOIN 
		...
		SELECT S.LOAI AS LOAI, SUM(SOTIEN) AS TongTien 
		        ...
		        GROUP BY S.LOAI
		UNION ALL
		        SELECT S.LOAI AS LOAI, SUM(S.SODU + COALESCE(PRT.SOTIEN, 0)) AS TongTien
		        FROM SOTIETKIEM S
		        LEFT JOIN PHIEURUTTIEN PRT
		        ON S.MASO = PRT.MASO
		        WHERE S.NGAYMOSO = p_NGAY
		        GROUP BY S.LOAI
		) AS RESULTS
    ...
    LEFT JOIN 
        (SELECT 
            S.LOAI, 
            SUM(PRT.SOTIEN + PRT.TIENLAI) AS TONGCHI
         FROM 
            PHIEURUTTIEN PRT
         JOIN 
