...
const getMonthlyRP = async (month, type) => {
  const [results, fields] = await connection.query("CALL BAOCAO_THANG(?,?)", [
    month,
    type,
  ]);
  return results[3];
...
const getNextMaSo = async () => {
  ...
  const [rows, fields] = await connection.query(
    "SELECT MAX(MASO) as maxMaSo FROM SOTIETKIEM"
  );
...
const getTenKH = async (MASO) => {
  const [results, fields] = await connection.query(
    "SELECT KH.HOTEN FROM SOTIETKIEM S JOIN KHACHHANG KH ON S.MAKH = KH.MAKH WHERE S.MASO = ?",
    [MASO]
  );
  if (results.length == 0) {
    return "";
...
const getLoaiTK = async (MASO) => {
  const [results, fields] = await connection.query(
    "SELECT LOAI FROM SOTIETKIEM WHERE MASO = ?",
    [MASO]
  );
  if (results.length > 0) return results[0].LOAI;
  else return -1;
};

const checkMaso = async (MASO) => {
  const [results, fields] = await connection.query(
    "SELECT COUNT(*) AS COUNT, TRANGTHAI FROM SOTIETKIEM WHERE MASO = ?",
    [MASO]
  );
...
const checkNgayRut = async (NGAY, MASO) => {
  ...
  if (results[0].DIFF >= ThoiGianToiThieu) return results[0].DIFF;
  return "ERROR";
};
...
const checkDaoHan = async (NGAY, MASO) => {
  ...
  if (results[0].DIFF / results[0].LOAI >= 30) return 1;
  return 0;
};
...
const calTienLai = async (MASO, NGAY, SOTIEN) => {
  ...
  if (LOAI == 0) {
    if (SOTIEN > SODU) return "ERROR 0";
    if (DIFF < 30) return 0;
    return SOTIEN * LAISUAT;
  } else {
    if (SOTIEN != SODU) return "ERROR 1";
    if (DAOHAN >= 1) return SOTIEN * LAISUAT * DAOHAN;
    else return "ERROR 2";
  }
};
...
const getSoDu = async (MASO) => {
  const [results, fields] = await connection.query(
    "SELECT SODU FROM SOTIETKIEM WHERE MASO = ?",
    [MASO]
  );
