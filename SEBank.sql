...
BEGIN
	...
	INSERT PHIEUGUITIEN (MAPHIEUGUI, MASO, NGAYGUI, SOTIEN) VALUES (@MAPHIEUGUI, @MASO, @NGAYGUI, @SOTIEN)
...
BEGIN
	...
	IF @LOAITK = 0 AND @SOTIEN <= (SELECT SODU FROM SOTIETKIEM WHERE @MASO = MASO ) 
		BEGIN
			...
			ELSE SET @TIENLAI = 0;
			INSERT PHIEURUTTIEN(MAPHIEURUT, MASO, NGAYRUT, SOTIEN, TIENLAI) VALUES (@MAPHIEURUT, @MASO, @NGAYRUT, @SOTIEN, @TIENLAI)
		END
	...
	UPDATE SOTIETKIEM
	SET TRANGTHAI = 0, NGAYDONGSO = @NGAYRUT
	WHERE SODU = 0 
...
GO
--Tra cứu sổ
CREATE OR ALTER PROCEDURE TRACUUSO
	@INPUT NVARCHAR(100)
AS
BEGIN
	SELECT STK.MASO, L.KYHAN,  KH.HOTEN, STK.SODU, STK.TRANGTHAI
	FROM SOTIETKIEM STK
	JOIN KHACHHANG KH ON KH.MAKH = STK.MAKH
	JOIN LOAI_SOTK L ON L.MALOAI = STK.LOAI
	WHERE @INPUT = KH.CMND OR @INPUT = STK.MASO OR KH.HOTEN LIKE '%' + @INPUT + '%'
	RETURN;
END
GO
--Báo cáo hoạt động ngày
CREATE OR ALTER PROCEDURE BAOCAO_NGAY
	@NGAY DATE
AS
BEGIN
	SELECT 
    L.KYHAN, 
    ISNULL(TONGTHU.TONGTHU, 0) AS TONGTHU, 
    ISNULL(TONGCHI.TONGCHI, 0) AS TONGCHI, 
    ISNULL(TONGTHU.TONGTHU, 0) - ISNULL(TONGCHI.TONGCHI, 0) AS CHENHLECH
	FROM 
		LOAI_SOTK L 
	LEFT JOIN 
		(SELECT S.LOAI, SUM(PGT.SOTIEN) AS TONGTHU
		 FROM PHIEUGUITIEN PGT
		 JOIN SOTIETKIEM S ON PGT.MASO = S.MASO
		 WHERE @NGAY = PGT.NGAYGUI
		 GROUP BY S.LOAI) AS TONGTHU 
	ON L.MALOAI = TONGTHU.LOAI 
	LEFT JOIN 
		(SELECT S.LOAI, SUM(PRT.SOTIEN) AS TONGCHI
		 FROM PHIEURUTTIEN PRT
		 JOIN SOTIETKIEM S ON PRT.MASO = S.MASO
		 WHERE @NGAY = PRT.NGAYRUT
		 GROUP BY S.LOAI) AS TONGCHI 
...
EXEC LAPPHIEUGUI 'MS000001', '20/07/2024',500000
EXEC LAPPHIEURUT 'MS000000', '18/10/2024',1000000
EXEC LAPPHIEURUT 'MS000001', '20/08/2024',500000
